...
# Simple Python Server (Port 10000 par)
...
# COMMENT HATA DIYA YA ALAG LINE MEIN LIKHA
EXPOSE 10000

# Start Script (Dono Services Ek Saath)
RUN echo '#!/bin/sh' > start.sh && \
    ...
    echo '' >> start.sh && \
    echo '# Phir Health Server chalao (FOREGROUND)' >> start.sh && \
